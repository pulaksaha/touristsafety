// PathSimulator.js - Adapted from Smart Route Monitoring App
...
class PathSimulator {
  constructor(routeCoordinates, updateLocationCallback, checkpoints, onSOS, onSimulationEnd, onTimerUpdate) {
    ...
    this.onTimerUpdate = onTimerUpdate; // Callback to update the timer
    this.currentIndex = 0;
    ...
    if (!this.timerInterval) {
      this.timerInterval = setInterval(() => {
        ...
        if (this.onTimerUpdate) {
          this.onTimerUpdate(elapsedSeconds); // Notify of the updated timer
        }
    ...
    if (this.currentCheckpointIndex < this.checkpoints.length) {
      ...
      if (distance <= threshold) {
        ...
        if (this.onCheckpointReached) {
          this.onCheckpointReached(this.currentCheckpointIndex); // Notify of checkpoint reached
        }
      }
    }
